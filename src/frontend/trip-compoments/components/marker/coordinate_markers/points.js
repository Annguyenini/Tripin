...
const CoordinatesPointsLayout = ({ trip_id, ready }) => {
  ...
  useEffect(() => {
    ...
    const setUpWatchList = async () => {
      let newCoords =
        await TripContentHandler.getTripCoordinatesHandler(trip_id);
      ...
      CurrentDisplayCoordinateObserver.setDefaultCoordsArray(
        trip_id,
        newCoords,
      );
      console.log("ready2");
    ...
    CurrentDisplayCoordinateObserver.attach(
      updateWatchList,
      CurrentDisplayCoordinateObserver.GENERATE_KEY(trip_id),
    );
    setUpWatchList();
    return () => {
      _cancelled = true;
  ...
  const coordinatesMap = useMemo(() => {
    return new Map([
      ...
      [
        20,
        [
          ...computeCluster(assestsObjectsArray, 20, true).map((obj) => {
      ...
      [
        40,
        [
          ...computeCluster(assestsObjectsArray, 40, true).map((obj) => {
            return [obj.center.lng, obj.center.lat];
          }),
        ],
      ],
      [
        60,
        [
          ...computeCluster(assestsObjectsArray, 60, true).map((obj) => {
            return [obj.center.lng, obj.center.lat];
          }),
        ],
      ],
      [
        80,
        [
          ...computeCluster(assestsObjectsArray, 80, true).map((obj) => {
            return [obj.center.lng, obj.center.lat];
          }),
        ],
      ],
      [
        100,
        [
          ...computeCluster(assestsObjectsArray, 100, true).map((obj) => {
            return [obj.center.lng, obj.center.lat];
          }),
        ],
      ],
