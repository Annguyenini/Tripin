...
export default function TrackingModeManager() {
  ...
  return (
    <View style={trackingModeManagerStyle.container}>
      <TouchableOpacity
        style={trackingModeManagerStyle.button}
        onPress={() => setVisible(true)}
      >
        <Text style={trackingModeManagerStyle.icon}>
          {" "}
          Mode {mode === TRACKING_MODE.NORMAL ? "🗺️" : "📷"}
        </Text>
          ...
          <OverlayCard onClose={() => setVisible(false)}>
            {isOnTheTrip && (
              <>
                ...
                >
                  <Text style={trackingModeManagerStyle.submitText}>Save</Text>
                </TouchableOpacity>
              </>
            )}
            {!isOnTheTrip && <Text>no trips yet</Text>}
          </OverlayCard>
        </Modal>
      )}
    </View>
