...
function RowItem({ icon, label, value, isLast, action }) {
  const { bg, iconColor } = ROW_ICONS[icon] || {
    bg: colors.peach,
...
export default function ProfileScreen({ onClose }) {
  ...
  return (
    <ScrollView
      style={profileStyle.screen}
      showsVerticalScrollIndicator={false}
    >
      <Text style={profileStyle.header}>profile</Text>
      <TouchableOpacity
        style={profileStyle.closeBtn}
        onPress={onClose}
        activeOpacity={0.8}
      >
      ...
      <View style={profileStyle.avatarSection}>
        <View style={profileStyle.avatarRing}>
          ...
          {!profileImageUri && (
            <Text style={profileStyle.avatarInitials}>TRIPPER</Text>
          )}
          <TouchableOpacity
            style={profileStyle.editAvatarBtn}
            activeOpacity={0.8}
      ...
      <Modal visible={showResetPassword}>
        <ResetPassword
