import {
  mediaCardStyle,
  deleteCardStyle,
} from "../../styles/function/media_card";
import {
  View,
  TouchableOpacity,
  Text,
...
export default function MediaViewCard({
  ...
  const [currentIndex, setCurrentIndex] = useState(
    Math.max(
      currentAssetsArray.findIndex((asset) => asset.media_path === uri),
      0,
    ),
  );
  ...
  console.log(
    "media",
    currentAssetsArray[currentIndex].media_path,
    currentIndex,
  );
  // ── ANIMATION ──
    ...
    return (
      <OverlayCard onClose={() => setDeleteVisible(false)}>
        ...
        <View style={deleteCardStyle.buttons}>
          <TouchableOpacity
            style={deleteCardStyle.cancelBtn}
            onPress={() => setDeleteVisible(false)}
          >
            <Text style={deleteCardStyle.cancelText}>Nooo</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={deleteCardStyle.confirmBtn}
            onPress={() => {
              deleteImageHandler();
      ...
      <GestureDetector gesture={changeMedia}>
        <View style={mediaCardStyle.overlayContainer}>
          ...
          >
            {propButton}
            <TouchableOpacity
              style={mediaCardStyle.exitButton}
              onPress={onClose}
            >
              <Text style={mediaCardStyle.exitText}>X</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={mediaCardStyle.dataButton}
              onPress={ImageDataDisplayHandler}
            >
              <Text style={mediaCardStyle.exitText}>...</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={mediaCardStyle.deleteButton}
              onPress={() => setDeleteVisible(true)}
            >
              <Text style={mediaCardStyle.exitText}>
                <MaterialIcons name="delete" size={24} color="red" />
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={mediaCardStyle.fullscreenButton}
          ...
          {dataVisible && (
            <MediaViewDataCard
              ...
              date={
                currentAssetsArray[currentIndex].time_stamp
                  ? currentAssetsArray[currentIndex].time_stamp
                  : currentAssetsArray[currentIndex].creationTime
              }
              visible={dataVisible}
                  ...
                  <View style={mediaCardStyle.clusterCard}>
                    <TouchableOpacity onPress={() => setCurrentIndex(index)}>
                      <Image
                        ...
                        style={mediaCardStyle.imageList}
                      />
                      {item.media_type === "video" && (
