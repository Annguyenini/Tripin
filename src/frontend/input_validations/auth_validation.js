...
export const validate = {
  ...
  oauthUsername: (v = "") => {
    const s = v.trim();
    return (
      s.length >= 3 && s.length <= 24 && /^[a-zA-Z][a-zA-Z0-9_]{2,23}$/.test(s)
    );
  },
  ...
  oauthDisplayName: (v = "") => {
    ...
    return (
      s.length >= 2 &&
      s.length <= 32 &&
      /^[\p{L}\p{N}][\p{L}\p{N}\s]{1,31}$/u.test(s)
    );
  },
  verifyCode: (v = "") => /^\d{6}$/.test(v.trim()),
...
export const MESSAGES = {
  ...
  oauthUsername:
    "Username must be 3–24 characters, start with a letter, letters, numbers, and underscores only.",
  password:
    "Password must be 8–15 characters, start with a letter, and include 1 uppercase, 1 number, and 1 special character (!@#$%^&*).",
  displayName:
    "Display name must be 5–10 characters, start with a letter, contain 1 uppercase, and no special characters.",
  oauthDisplayName:
    "Display name must be 2–32 characters, letters and numbers only.",
...
export function validateSignup({
  ...
  if (!validate.passwordsMatch(password, confirmPassword))
    errors.push(MESSAGES.passwordsMatch);
  return errors;
}

export function validateLogin({ username, password, email }) {
  const errors = [];
  if ((!username?.trim() && !email?.trim()) || !password?.trim())
    errors.push(MESSAGES.required);
  if (username) {
    if (!validate.username(username)) {
      errors.push(MESSAGES.username);
    }
  } else if (email) {
...
export function validateOAuthComplete({
  username,
  displayName,
  password,
  confirmPassword,
  ...
  if (!validate.oauthDisplayName(displayName))
    errors.push(MESSAGES.oauthDisplayName);
  if (!validate.password(password)) errors.push(MESSAGES.password);
  if (!validate.passwordsMatch(password, confirmPassword))
    errors.push(MESSAGES.passwordsMatch);
