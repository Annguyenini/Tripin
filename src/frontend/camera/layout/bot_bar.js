...
const BotBarControl = ({
  ...
  return (
    <View style={cameraStyle.botControls}>
      ...
      <TouchableOpacity
        style={cameraStyle.outerCircle}
        onPress={click}
        onLongPress={LongClick}
        onPressOut={stopLongClick}
        activeOpacity={0.7}
        delayLongPress={300}
      >
        <Text
          style={[
            cameraStyle.innerCircle,
            { backgroundColor: currentMode === "photo" ? "white" : "red" },
          ]}
        >
          {" "}
        </Text>
      ...
      >
        <Image
          source={{ uri: image_icon }}
          style={[cameraStyle.icon, { left: 40, width: 50, height: 50 }]}
      ...
      {visible && (
        <MediaViewCard
          ...
          AssetArray={AlbumService.AlbumsArray}
        ></MediaViewCard>
      )}
    </View>
  );
