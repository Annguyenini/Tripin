...
class TripContentSyncManager {
  ...
  async _getAndCompareTripMediasHash(trip_id) {
    const response = await safeRun(
      () => TripContentsSyncService.requestTripMediasHash(trip_id),
      "failed_at_get_trip_media_hash_from_server",
    );
    if (!response.ok || response.status !== 200 || !response.data.hash)
      return false;

    const local_hash = await safeRun(
      () => Albumdb.getMediaHash(trip_id),
      "failed to get max modified time",
    );
  ...
  async _getAndProcessTripMediasMetadata(trip_id) {
    const response = await safeRun(
      () => TripContentsSyncService.requestTripMediasMetadata(trip_id),
    ...
    const local_trip_media_assets = await safeRun(
      () => Albumdb.getAssestsFromTripId(trip_id),
  ...
  async tripCoordinateSync(trip_id) {
    const respond = await safeRun(
      () => TripContentsSyncService.requestTripCoordinateHash(trip_id),
      "failed_at_request_trip_coordinate_hash",
    );
    ...
    const local_hash = await safeRun(
      ...
      "failed_at_get_trip_coordinate_version",
    );
    ...
    await safeRun(
      () => TripContentsSync.processTripCoordinatesSync(trip_id),
      "failed_at_process_trip_coordinate_sync",
    );
    if (_onCallBack) _onCallBack(false);
  }
  ...
  async _processRequestDeleteTripMedias(trip_id, delete_array) {
    ...
    await safeRun(
      () => TripContentsSync.process(),
      "failed_at_process_delete_media_sync",
    );
    return;
  }
  ...
  async _processRequestUploadTripMedias(trip_id, upload_array) {
    ...
    await safeRun(
      () => TripContentsSync.process(),
      "failed_at_process_upload_media_sync",
    );
    return;
  }
  ...
  async tripMediaSyncHandler(trip_id) {
    ...
    if (!compare_hash) {
      await safeRun(
        () => this._getAndProcessTripMediasMetadata(trip_id),
        "failed_at_trip_media_sync_manager",
      );
    }
    if (_onCallBack) _onCallBack(false);
    return;
