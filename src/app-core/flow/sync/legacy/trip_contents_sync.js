...
class TripSync {
    ...
    while (this.pennding.length !== 0) {
      ...
      switch (item.itemType) {
        ...
        case "delete_coordinate":
          await TripContentsService.deleteCoordinate(
            item.data.trip_id,
            item.data.coordinate_id,
            item.data.modified_time,
          );
          break;
        case "media":
          await TripContentsService.sendTripMedia(
            item.data.media_id,
            CurrentTripDataService.getCurrentTripId(),
            item.data.media_path,
            item.data.longitude,
            item.data.latitude,
            ...
            item.data.region,
            item.data.country,
            item.data.iso_country_code,
          );
          break;

        case "delete_media":
          await TripContentsService.deleteMedias(
            item.data.trip_id,
            item.data.media_id,
            item.data.modified_time,
          );
  ...
  async processTripCoordinatesSync(trip_id) {
    ...
    const response = await safeRun(() =>
      TripContentsService.requestTripCoordinates(trip_id),
    );
    const server_coordinate = response.data.coordinates;
    const local_coordinate = await safeRun(() =>
      this.TripCoordinateDatabase.getAllCoordinatesFromTripId(trip_id),
    );

    const delete_array =
      server_coordinate?.filter((server) => {
        return local_coordinate?.find(
          (local) =>
    ...
    const upload_array =
      local_coordinate?.filter((local) => {
        return !server_coordinate?.find(
          (server) => server.coordinate_id === local.coordinate_id,
        );
      }) || [];
    const download_array =
      server_coordinate?.filter((server) => {
        return !local_coordinate?.find(
          (local) => local.coordinate_id === server.coordinate_id,
        );
      }) || [];
    this.coordinatesSyncing = true;
    if (download_array)
      await this._processDownloadCoordinate(download_array, trip_id);
    ...
    await safeRun(
      () => this.process(),
      "failed_at_process_sync_trip_coordinate",
    );
  }
  async _processDownloadCoordinate(localArray, trip_id) {
    await safeRun(() =>
      this.TripCoordinateDatabase.handlerCoordinateFromServer(
        localArray,
        trip_id,
      ),
    );
