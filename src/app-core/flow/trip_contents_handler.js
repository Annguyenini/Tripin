...
class TripContentHandler {
    ...
    async uploadTripMediaHandler(media_id, trip_id, media_uri, longitude, latitude, coordinate_id, type, time_stamp, city, region, country, iso_country_code) {
        if (!media_uri) return
        this._uploadPending = (this._uploadPending || 0) + 1
        try {
            // pending count to prevent spam lead to loop sync
            // send to server
            const respond = await safeRun(() => TripContents.sendTripMedia(media_id, trip_id, media_uri, longitude, latitude, type, coordinate_id, time_stamp, city, region, country, iso_country_code), 'failed_at_send_media_to_server')
