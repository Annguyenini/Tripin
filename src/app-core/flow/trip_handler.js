...
class TripHandler {
  ...
  async requestNewTripHandler(trip_name, imageUri = null) {
    try {
      ...
      const respond = await Trip.requestNewTrip(
        trip_name,
        created_time,
        imageUri,
      );
      ...
      // tripdata object
      const trip_data = CurrentTripDataService.getObjectReady(
        user_id,
        trip_id,
        trip_name,
        created_time,
    ...
    if (trip_id) {
      ...
      if (status === 304) {
        if (
          ...
          // if load from local failed, delete etag and request it again
          await safeRun(
            () =>
