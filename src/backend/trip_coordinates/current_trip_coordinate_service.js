...
class CurrentTripCoordinateService extends TripCoordinateDatabase {
  ...
  async insert_into_DB(temp_storage) {
    ...
    const current_version =
      (await TripDatabase.getTripCoordinateVersion(
    ...
    await DB.withTransactionAsync(async () => {
      for (const item of temp_storage) {
        try {
          await DB.runAsync(
            `INSERT INTO trip_${CurrentTripDataService.getCurrentTripId()} (altitude, latitude, longitude,heading,speed,time_stamp,version,event,coordinate_id,modified_time) VALUES (?,?,?,?,?,?,?,?,?,?)`,
            [
              item.altitude,
              item.latitude,
              item.longitude,
  ...
  async coordinatesStorageHandler() {
    ...
    await TripDatabase.updateTripCorrdinateVersion(
      CurrentTripDataService.getCurrentTripId(),
      version,
    );

    const send_coor = await TripContentsHandler.sendCoordinatesHandler(
      temp_storage,
      version,
    );
