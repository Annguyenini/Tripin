...
class Album {
  ...
  async deleteMediaFromDB(media_id, trip_id, modified_time) {
    ...
    try {
      await DB.runAsync(
        `UPDATE user_${UserDataService.getUserId()}_album SET event = ? , modified_time =? WHERE media_id = ?`,
        "remove",
        modified_time,
        media_id,
      );
  ...
  async getAssestsFromTripId(trip_id) {
    try {
      ...
      const result = await DB.getAllAsync(
        `SELECT * FROM user_${UserDataService.getUserId()}_album WHERE trip_id = ?`,
        [trip_id],
      );
      return result;
    } catch (err) {
  ...
  async getAllMediasFromDbTest() {
    ...
    try {
      result = await DB.getAllAsync(
        `SELECT * FROM user_${UserDataService.getUserId()}_album ORDER BY time_stamp DESC`,
      );
      return result;
    } catch (err) {
      console.error(err);
  ...
  async getAllMediasFromDb() {
    ...
    try {
      result = await DB.getAllAsync(
        `SELECT * FROM user_${UserDataService.getUserId()}_album WHERE event = ? ORDER BY time_stamp DESC`,
        "add",
      );
      return result;
    } catch (err) {
