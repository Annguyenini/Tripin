...
class HashService extends LocalStorage {
  ...
  async saveHashToLocalStorage(trip_id, type, hash) {
    await safeRun(
      () => this.saveToLocal(this.GENERATE_TRIP_HASH_KEY(trip_id, type), hash),
      "failed_at_save_hash_to_local",
    );
  }

  async getHashFromLocalStorage(trip_id, type) {
    return await safeRun(
      () => this.getDataFromLocal(this.GENERATE_TRIP_HASH_KEY(trip_id, type)),
      "failed_at_get_hash_from_local",
    );
  }
  //
  /**
  ...
  async generateAndSaveTripMediaHash(trip_id) {
    const media_metadata = await safeRun(() =>
      Albumdb.getAssestsFromTripId(trip_id),
    );
    const local_hash = await safeRun(
      () => this._generateLocalTripMediaHash(media_metadata),
      "failed_at_generate_trip_media_hash",
    );
    if (local_hash) {
      await safeRun(
        () => this.saveHashToLocalStorage(trip_id, "trip_media", local_hash),
        "failed_at_save_hash_to_local",
      );
    }
    return local_hash;
