...
class Permission extends LocalStorage {
  ...
  async setForeGroundPer(status) {
    ...
    try {
      await safeRun(
        () =>
          this.saveToLocal(STORAGE_KEYS.SETTINGS.FOREGROUNDPERMISSION, status),
        "failed_at_save_foreground_to_local",
      );
    } catch (asyncError) {
  ...
  async setBackGroundPer(status) {
    try {
      await safeRun(
        () =>
          this.saveToLocal(STORAGE_KEYS.SETTINGS.BACKGROUNDPERMISSION, status),
        "failed_at_save_background_to_local",
      );
    } catch (asyncError) {
  ...
  async setCameraPer(status) {
    ...
    try {
      await safeRun(
        () => this.saveToLocal(STORAGE_KEYS.SETTINGS.CAMERAPERMISSION, status),
        "failed_at_save_camera_to_local",
      );
    } catch (asyncError) {
  ...
  async setAlbumPer(status) {
    ...
    try {
      await safeRun(
        () => this.saveToLocal(STORAGE_KEYS.SETTINGS.ALBUMPERMISSION, status),
        "failed_at_save_camera_to_local",
      );
    } catch (asyncError) {
  ...
  async setInitSettings(status) {
    ...
    try {
      await safeRun(
        () => this.saveToLocal(STORAGE_KEYS.SETTINGS.INITSETTINGS, status),
        "failed_at_save_init_setting_to_local",
      );
    } catch (asyncError) {
  ...
  async getForeGroundPer() {
    try {
      const status = await safeRun(
        () => this.getDataFromLocal(STORAGE_KEYS.SETTINGS.FOREGROUNDPERMISSION),
        "failed_at_get_foreground_setting",
      );
      return status;
    } catch (asyncError) {
  ...
  async getBackGroundPer() {
    try {
      const status = await safeRun(
        () => this.getDataFromLocal(STORAGE_KEYS.SETTINGS.BACKGROUNDPERMISSION),
        "failed_at_get_background_setting",
      );
      return status;
    } catch (asyncError) {
      console.error`ERROR to get FOREGROUND Value`;
  ...
  async getCameraPer() {
    try {
      const status = await safeRun(
        () => this.getDataFromLocal(STORAGE_KEYS.SETTINGS.CAMERAPERMISSION),
        "failed_at_get_foreground_setting",
      );
      return status;
    } catch (asyncError) {
  ...
  async getAlbumPer() {
    try {
      const status = await safeRun(
        () => this.getDataFromLocal(STORAGE_KEYS.SETTINGS.ALBUMPERMISSION),
        "failed_at_get_background_setting",
      );
      return status;
    } catch (asyncError) {
  ...
  async getInitSettingPer() {
    try {
      const status = await safeRun(
        () => this.getDataFromLocal(STORAGE_KEYS.SETTINGS.INITSETTINGS),
        "failed_at_get_init_setting_setting",
      );
      return status;
    } catch (asyncError) {
