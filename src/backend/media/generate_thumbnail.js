...
export const generateOrGetThumbnailFromMediaId = async (
  media_id,
  media_path,
) => {
  let outsource = false;
  ...
  try {
    ...
    await makeDirectoryAsync(`${documentDirectory}thumbnails/`, {
      intermediates: true,
    });
    await copyAsync({ from: thumbnailUri, to: dest });
    return dest;
  } catch (err) {
    console.error("failed to create thumbnail", err.stack, err.message);
    return null;
  }
};
